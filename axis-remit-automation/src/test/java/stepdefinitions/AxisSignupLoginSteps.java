...
import pages.AxisLoginPage;
import pages.AxisSignupPage;
import utils.ExcelUtils;
import utils.FakerUtils;
import utils.ConfigReader;

public class AxisSignupLoginSteps {

	WebDriver driver = BaseTest.getDriver();
	AxisSignupPage signupPage;
	AxisLoginPage loginPage;
	...
	public void user_opens_the_axis_remit_page() throws Exception {
		...
		testData = ExcelUtils.getTestData(3);

		driver.get(testData.get("baseUrl"));
		signupPage = new AxisSignupPage(driver);
		loginPage = new AxisLoginPage(driver);
	}
...
//	}
	...
	public void user_selects_country() {
		...
		String euroCountry = testData.get("euroCountry");

		System.out.println("Selecting country: " + country);

		// Always select main country
		signupPage.selectCountry(country);

		// ✅ Select euro country ONLY for Eurozone
		if ("Eurozone".equalsIgnoreCase(country) && euroCountry != null && !euroCountry.isBlank()) {

			System.out.println("Eurozone detected, selecting euro country: " + euroCountry);
			signupPage.selectEuroCountry(euroCountry);

		} else {
			System.out.println("No euro country selection required for: " + country);
		}
	}
...
//	}
	...
	public void user_selects_option() {
		...
		String country = testData.get("country");
		String option = testData.get("option");

		// ✅ USA does NOT have option selection
		if ("United States of America".equalsIgnoreCase(country)) {
			System.out.println("Option not applicable for USA. Skipping.");
			return;
		}

		// ✅ Safety: skip if option value not provided
		if (option == null || option.isBlank()) {
			System.out.println("Option value is blank. Skipping.");
			return;
		}

		// ✅ Extra safety: check if option element exists before clicking
		if (!signupPage.isOptionPresent(option)) {
			System.out.println("Option '" + option + "' not present on UI. Skipping.");
			return;
		}
